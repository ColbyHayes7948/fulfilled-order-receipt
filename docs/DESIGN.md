...
> Design notes for **fulfilled-order-receipt** — a runnable python example that A typed Python service that sends one receipt when an ecommerce order is fulfilled.
...
- **Feature code** — the actual task: order-receipt-email.
